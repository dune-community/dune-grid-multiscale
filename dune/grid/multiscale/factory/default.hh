...
public:
  ...
  void report(std::ostream& out) {
    auto fmt = boost::format("entity index %s in subdomain %s\n");
    for (auto&& item : *entityToSubdomainMap_) {
      out << fmt % item.first % item.second ;
    }
    out << std::endl;
  }
           ...
           std::ostream& out = Dune::Stuff::Common::Logger().debug())
  {
    ...
    } else if (indexIt->second != subdomain) {
      std::stringstream msg;
      msg << "Error in " << id() << ": can not add entity to more than one subdomain!";
      DUNE_THROW(Dune::InvalidStateException, msg.str());
    } else {
      DSC_LOG_DEBUG << "globalIndex " << globalIndex << "already in map" << std::endl;
    }
                ...
                bool assert_connected = true)
  {
    ...
    for (size_t subdomain = 0; subdomain < size_; ++subdomain) {
      ...
      for (auto&& subdomainGeometryMapIt : subdomainGeometryMap) {
        // get geometry
    ...
    // walk the global grid part to generate the information which sudomains neighbor each other
